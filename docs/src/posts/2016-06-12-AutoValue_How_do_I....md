...
最佳实践：标记你的俯冲方法（underriding methods）为 <code>final</code>类型来让以后阅读代码的人知道这些方法没有被 AutoValue 重写。
...
这两种情况的字段都会在<code>equals</code><code>hashCode</code><code>toString</code>方法中忽略，对 AutoValue 来说这个字段根本不存在。
